...
export default function ApproverPage() {
  ...
  useEffect(() => {
    ...
    if (t) setDocTypeFilter(t)
  }, [searchParams])

    useEffect(() => {
    if (!documents.length) return
    if (docTypeFilter || fromDate || toDate || titleFilter) handleSearch()
...
const handleSearch = () => {
  ...
  if (docTypeFilter) {
    filtered = filtered.filter((doc) => normalize(doc.docType) === normalize(docTypeFilter))
  }

  if (titleFilter) {
    filtered = filtered.filter((doc) =>
      normalize(doc.docName).includes(normalize(titleFilter))
    )
  }
  ...
  const handleApprove = async () => {
    ...
    await fetch(`/api/document/approve/${selectedDoc.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ approverUsername: userEmail }),
    })
  ...
  const isAlreadyApproved = (doc: any) => {
    return doc.approval1By === userEmail || doc.approval2By === userEmail
  }
        ...
        <MainLayout isOpen={isOpen}>
          <div className="bg-white shadow-md rounded-md p-6 relative overflow-x-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Approval Document Page</h2>
            </div>
            ...
            {showModal && selectedDoc && (
              <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
                <div className="bg-white rounded shadow-lg p-6 w-full max-w-xl">
                  <h2 className="text-lg font-semibold mb-4">Form Entry Document</h2>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <label className="font-bold text-black">Document No</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-gray-200">{selectedDoc.documentNo}</div>
                    </div>
                    <div>
                      <label className="font-bold text-black">ISO Type</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-white">{selectedDoc.isoType}</div>
                    </div>
                    <div>
                      <label className="font-bold text-black">Request From</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-gray-200">{selectedDoc.requestFrom}</div>
                    </div>
                    <div>
                      <label className="font-bold text-black">Dept</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-gray-200">{selectedDoc.dept}</div>
                    </div>
                    <div className="col-span-2">
                      <label className="font-bold text-black">Doc. Type</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-white">{selectedDoc.docType}</div>
                    </div>
                    <div className="col-span-2">
                      <label className="font-bold text-black">Doc. Name / Title</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-white">{selectedDoc.docName}</div>
                    </div>
                    <div className="col-span-2">
                      <label className="font-bold text-black">Objective</label>
                      <div className="mt-1 p-2 border border-gray-300 rounded bg-white">{selectedDoc.objective}</div>
                    </div>
                    <div className="col-span-2">
                      <label className="font-bold text-black">Attached File : </label>
                      <a href={selectedDoc.attachmentPath} target="_blank" className="text-blue-700 underline">
                  ...
                  <div className="mt-6 flex justify-end space-x-2">
                    ...
                    {isAlreadyApproved(selectedDoc) ? (
                      <button disabled className="bg-gray-300 text-gray-700 px-4 py-2 rounded cursor-not-allowed">Already Approved</button>
                    ) : (
