...
export default function AdminDashboard() {
  ...
  const goTo = (k: keyof typeof typeLabel) => {
    router.push(`/dashboard/admin/approval?type=${encodeURIComponent(typeLabel[k])}`)
  }
  ...
  useEffect(() => {
    fetch('/api/document/summary-count')
      .then(res => res.json())
      .then((data) => {
        const mapped: Stats = { SOP: 0, INSTRUKSI_KERJA: 0, FORMULIR: 0, LAPORAN: 0 }
        data.forEach((item: { docType: string; _count: { docType: number } }) => {
          ...
          if (key in mapped) mapped[key] = item._count.docType
        })
        setStats(mapped)
      })
      ...
      <div className="flex-1 flex flex-col">
        <MainLayout isOpen={isOpen}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6">
            <div className="bg-cyan-600 text-white rounded-lg p-4 shadow-md hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out cursor-pointer">
              <div className="flex justify-between items-center">
                <div className="text-3xl font-bold">{stats.SOP}</div>
                <FaFileAlt size={28} />
              </div>
              <div className="mt-2 text-sm font-semibold">SOP</div>
              <button onClick={() => goTo('SOP')} className="text-xs mt-1 underline underline-offset-4">More info →</button>
            </div>

            <div className="bg-green-600 text-white rounded-lg p-4 shadow-md hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out cursor-pointer">
              <div className="flex justify-between items-center">
                <div className="text-3xl font-bold">{stats.INSTRUKSI_KERJA}</div>
                <FaChartBar size={28} />
              </div>
              <div className="mt-2 text-sm font-semibold">INSTRUKSI KERJA</div>
              <button onClick={() => goTo('INSTRUKSI_KERJA')} className="text-xs mt-1 underline underline-offset-4">More info →</button>
            </div>

            <div className="bg-yellow-500 text-white rounded-lg p-4 shadow-md hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out cursor-pointer">
              <div className="flex justify-between items-center">
                <div className="text-3xl font-bold">{stats.FORMULIR}</div>
                <FaUserPlus size={28} />
              </div>
              <div className="mt-2 text-sm font-semibold">FORMULIR</div>
              <button onClick={() => goTo('FORMULIR')} className="text-xs mt-1 underline underline-offset-4">More info →</button>
            </div>

            <div className="bg-red-600 text-white rounded-lg p-4 shadow-md hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out cursor-pointer">
              <div className="flex justify-between items-center">
