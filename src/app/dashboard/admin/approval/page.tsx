...
export default function AdminDocumentPage() {
  ...
  const handleApprove = async (type: '1' | '2') => {
    if (!selectedDoc) return
    await fetch(`/api/document/approve/${selectedDoc.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        approverUsername: userEmail,
        forceLevel: type,
      }),
    })
    setShowModal(false)
        ...
        <MainLayout isOpen={isOpen}>
          <div className="bg-white shadow-md rounded-md p-6 relative overflow-x-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Document List</h2>
            </div>
            ...
            {showModal && selectedDoc && (
              <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
                <div className="bg-white rounded shadow-lg p-6 w-full max-w-xl">
                  <h2 className="text-lg font-semibold mb-4">Admin Review & Approve</h2>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="col-span-2"><label>Doc. Name</label><div className="mt-1 p-2 border border-gray-300 rounded bg-white">{selectedDoc.docName}</div></div>
                  ...
                  <div className="mt-6 flex justify-end space-x-2">
                    <button onClick={() => setShowModal(false)} className="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded">Close</button>
                    ...
                    <button
                      disabled
                      className="bg-yellow-300 text-white px-4 py-2 rounded cursor-not-allowed"
                    >
                      Waiting Review
                    </button>
                  ) : (
